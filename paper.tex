...
\begin{table}[h]
\centering
\caption{Generalization. $-$nudge and $-$no-tools relative to full Harness, on two additional settings.}
\label{tab:cross}
\begin{tabular}{llrrr}
\toprule
...
\begin{itemize}
  \item \textbf{No-tools is never worse than full Harness, and is often substantially better.} +3.50pp on SVAMP, +15.62pp on Llama. The GSM8K-on-Qwen result (the original setting) is the \emph{least} striking case; the pattern is more extreme on harder/cross-family transfer.
  \item \textbf{The $-$nudge effect is specific to the PASS-spiral failure mode.} On SVAMP, where the model rarely spirals, $-$nudge $\approx$ full (within $0.10$pp). On Llama, where the model's tool calls are weaker but the model still calls validate at 89.2\% rate, $-$nudge $\approx$ full ($-$0.31pp); the spiral mode is not what dominates here---tool exposure itself is poison.
\end{itemize}

Taken together, the three settings rule out the explanation that ``tools help, just less than people thought.'' Across all three, the tools' causal contribution to accuracy is either zero (Qwen GSM8K) or negative (Qwen SVAMP, Llama GSM8K).
...
\begin{enumerate}
  ...
  \item \textbf{On the harder transfer settings, removing the tools \emph{improves} accuracy substantially}: +3.50pp on SVAMP, +15.62pp on Llama.
