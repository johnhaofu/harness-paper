...
\midrule
GSM8K (n=1319) & Qwen3.5-4B (teacher, no OPD/GRPO) & 94.16 & --- & 94.01 \\
MATH-500 (n=500) & Qwen3.5-4B (teacher) & 75.20 & --- & \textbf{84.00} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
  \item \textbf{No-tools is never worse than full Harness, and is often substantially better.} +3.50pp on SVAMP, +3.40pp on MATH-500 (Qwen 2B), +15.62pp on Llama. On the larger 4B teacher the no-tools advantage on MATH-500 grows to +8.80pp; on GSM8K the two are within noise (94.01 vs 94.16). The GSM8K-on-Qwen-2B result (the original setting) is the \emph{least} striking case; the pattern is more extreme on harder benchmarks, on cross-family transfer, and on the larger same-family model.
...
Taken together, the settings rule out the explanation that ``tools help, just less than people thought.'' Across all six configurations the tools' causal contribution to accuracy is either zero (Qwen 2B GSM8K, Qwen 4B GSM8K) or negative (Qwen 2B SVAMP, Qwen 2B MATH-500, Qwen 4B MATH-500, Llama 3B GSM8K). The 4B-teacher numbers extend the finding upward in scale: a roughly 2$\times$-larger same-family model with a stronger base CoT still gains nothing from the tools on GSM8K and loses 8.80pp on MATH-500 by carrying them.

\paragraph{Capability-floor note: AIME 2024/2025 on the 2B model.} We also ran the same three configurations on AIME 2024 and AIME 2025 (combined $n{=}60$ problems, $30$ each), where Qwen3.5-2B is far below ceiling: pooled accuracy is $21.7\%$ for full Harness, $20.0\%$ for $-$nudge, and $18.3\%$ for $-$no-tools. The pooled differences are well below the binomial standard error at $n{=}30$ per split ($\sigma \approx 7.3$pp), and on the individual splits the direction of the nudge effect flips between 2024 (where full $>$ $-$nudge by $10$pp) and 2025 (where $-$nudge $>$ full by $7$pp). We interpret this as a capability-floor regime where the model's success rate is dominated by the small fraction of problems it can solve at all, and the choice of scaffolding mostly reshuffles which $\sim$5--8 of 30 problems it happens to get; the ablation framework we use does not deliver a useful signal in this regime. We do not include AIME in the main claim of the paper.

For size-controlled context, Table~\ref{tab:aime-context} places our 2B+OPD+GRPO+Harness AIME 2024 number (26.67\%, $8/30$) against reported AIME 2024 scores for other models. The single-seed $n{=}30$ result places our 2B system above GPT-4o and below the substantially larger and math-specialized Qwen2.5-Math-72B (greedy). We emphasize the $\sigma \approx 7$pp uncertainty: the difference between rows within a $\sim$10pp window is not statistically distinguishable from our single-seed data, and the comparison is included only to anchor the absolute scale of our numbers, not to argue ranking.

\begin{table}[h]
\centering
\caption{AIME 2024 size-controlled context. Our 2B number is single-seed, $n{=}30$, $\sigma{\approx}7$pp. External numbers are as reported by the cited sources; methodology (greedy / pass@1 / cons@k) varies across sources.}
\label{tab:aime-context}
\begin{tabular}{lrl}
\toprule
...
\textbf{Qwen3.5-2B+OPD+GRPO+Harness (ours)} & \textbf{26.67\%} & this work, $n{=}30$, single seed \\
...
\paragraph{4B teacher numbers on AIME.} For completeness we also ran the three configurations on the un-trained Qwen3.5-4B teacher (no OPD, no GRPO, the same base used as our distillation teacher). Pooled over AIME 2024+2025 ($n{=}60$), the 4B teacher with Harness reaches $61.7\%$, the no-nudge variant $61.7\%$, and the no-tools variant $56.7\%$. The 4B is above the AIME capability floor in a way the 2B is not, and the no-tools result is again the lowest of the three. We treat this as further weak evidence that, once the model is competent enough to engage AIME at all, tools do not contribute and may slightly help relative to no-tools---a regime that is the opposite of our main finding and that deserves its own study (see \S\ref{sec:limitations}).
